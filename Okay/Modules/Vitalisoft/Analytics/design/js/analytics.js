/* VIEW_ITEM_LIST */
function isHrefPresent(product) {
    const el = document.querySelector(`[href*="${product.href}"]`)
    return !!(el && el.offsetParent)
}
document.addEventListener('DOMContentLoaded', function () {
    if (route_name == 'user_favorites') {
        if (okay.products && okay.products.length) {
            const list = okay.products.filter(product => product.item_list_name == 'User Wishlist' && isHrefPresent(product))
            view_item_list(list);
        }
    }
    if (route_name == 'user_browsed') {
        if (okay.products && okay.products.length) {
            const list = okay.products.filter(product => product.item_list_name == 'User Browsed Products' && isHrefPresent(product))
            view_item_list(list);
        }
    }
    ...
    if (['order', 'cart', 'user', 'user_browsed', 'user_favorites'].indexOf(route_name) < 0 && okay.products && okay.products.length) {
        const list = okay.products.filter(product => product.item_list_name && isHrefPresent(product));
        view_item_list(list);
    }
});
function view_item_list(list) {
    if (ga4_analytics && list && list.length)
        dataLayer.push({
...
if (ga4_analytics && ['order', 'cart'].indexOf(route_name) < 0) {
    $(document).on('click', '.product_preview__image a, .product_preview__name_link, a[data-product]', function (e) {
        if ($(this).hasClass('fn_wishlist')) {
            return; // Прервать выполнение обработчика, если элемент содержит класс "fn_wishlist"
        }
