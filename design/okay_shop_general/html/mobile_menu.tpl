<ul class="top-nav">
    <li>
        ...
        <div class="d-flex align-items-center f_col">
            {if $user}
                <a class="account__link d-inline-flex align-items-center icon icon-perm-identity"
                    href="{url_generator route="user"}">
                    <span class="account__text" data-language="index_account">{$lang->index_account} </span>
                    {$user->name|escape}
                </a>
            {else}
                <a class="account__link d-inline-flex align-items-center icon icon-perm-identity"
                    href="{url_generator route='login'}" title="{$lang->index_login}">
                    <span class="account__text" data-language="index_account">{$lang->index_account} </span>
                    ...
                    {foreach $categories as $c}
                        {if $c->visible && ($c->has_products || $settings->show_empty_categories)}
                            {if $c->subcategories && $c->count_children_visible}
                                <li class="">
                                    <a class="{if $category->id == $c->id} selected{/if}"
                                        href="{url_generator route='category' url=$c->url}" data-category="{$c->id}">
                                        {if $c->image}
                                            <span class="nav-icon">
                                                <img src="{$c->image|resize:20:20:false:$config->resized_categories_dir}"
                                                    alt="{$c->name|escape}" />
                                            </span>
                                        {/if}
...
{if $currencies|count > 1}
    <ul class="currencies-nav">
        <li class="">
            ...
            <span class=""><span data-language="mobile_menu_currency">{$lang->mobile_menu_currency}:</span>
                {$currency->name}</span>
            <ul class="">
                            ...
                            <li>
                                <a class=" {if $language->id == $l->id} active{/if}" href="{preg_replace('/^(.+)\/$/', '$1', $l->url)}">
                                    {if is_file("{$config->lang_images_dir}{$l->label}.png")}
                                        <img alt="{$l->current_name}" src="{("{$l->label}.png"
                        )|resize:20:20:false:$config->lang_resized_dir}" />
                                    {/if}
