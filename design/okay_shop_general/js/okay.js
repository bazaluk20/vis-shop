...
$(function () {
  ...
  $("input[name='callback_phone']").inputmask({
  ...
  if ($(".gallery-thumbs").length) {
    ...
    var galleryTop = new Swiper(".gallery-top", {
      ...
      pagination: {
        el: this.querySelector(".swiper-pagination-gallery_image"),
        clickable: true,
      },
      thumbs: {
...
// Получаем элементы
const countMonthEl = document.querySelector('.paypart__count_month .count_month');
const priceEl = document.querySelector('.fn_price');
const resultEl = document.querySelector('.privat_oplata_chastynamy');
const resultBtn = document.querySelector('.privat_oplata_chastynamy_btn');

// Проверяем, что все элементы существуют
if (countMonthEl && priceEl && resultEl && resultBtn) {
    // Проверяем наличие текста в блоке count_month
    const countMonthText = countMonthEl.textContent.trim();
    const priceText = priceEl.textContent.trim().replace(/\s+/g, '').replace(',', '.');

    if (countMonthText && priceText) {
        const countMonth = parseInt(countMonthText, 10);
        const price = parseFloat(priceText);

        // Проверяем корректность чисел
        if (!isNaN(countMonth) && !isNaN(price) && countMonth > 0) {
            // Выполняем расчет
            const result = (price / countMonth + 1).toFixed(0);

            // Записываем результат и показываем кнопку
            resultEl.textContent = result;
            resultBtn.classList.remove('hidden');
        } else {
            console.error('Некорректные данные: countMonth или price.');
        }
    } else {
        console.warn('Тексты countMonth или price пусты.');
    }
} else {
...
document.addEventListener('DOMContentLoaded', () => {
  // Получаем элементы
  const sortTitle = document.querySelector('.product_sort__title');
  const sortFormWrapper = document.querySelector('.sort-form-wrapper');

  // Проверяем наличие элементов
  if (sortTitle && sortFormWrapper) {
      sortTitle.addEventListener('click', () => {
          // Проверяем ширину экрана
          if (window.innerWidth < 768) {
              // Переключаем класс
              sortFormWrapper.classList.toggle('hidden-sm-down');
          }
      });
  }
});
