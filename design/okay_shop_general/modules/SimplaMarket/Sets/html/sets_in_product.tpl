                            ...
                            <div class="product_set__item_wrapper">
                                ...
                                {foreach $set->items as $item}
                                    <div class="product_set_item">
                                        ...
                                        <div class="product_set_item__content">
                                            <a class="product_set_item__name item_var"
                                                href="{url_generator route="product" url=$item->product->url}">{$item->product->name}</a>
                                            {$product_author}
                                        ...
                                        <div class="product_set_item__image">
                                            <a class="product_set_item__name item_var"
                                                href="{url_generator route="product" url=$item->product->url}"><img
                                                    src="{$item->product->image->filename|resize:200:240}" /></a>
                                        </div>
                                    </div>
                                    {if !$item@last}
                                        <div class="product_set__sign">
                                            <svg style="display: block" width="38" height="38" viewBox="0 0 38 38" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path fill-rule="evenodd" clip-rule="evenodd"
                                                    d="M38 20H20V38H18V20H0V18H18V0H20V18H38V20Z" fill="#4F4F4F" />
                                            </svg>
                                        </div>
                                    {/if}
                                {/foreach}
                                ...
                                <div class="product_set_buy">
                                    <div class="product_set_buy__prices">
                                        <span
                                            class="product_set_buy__price set_new_price  {if $set->total_discount } price--red{/if}">{$set->total_price|convert}
                                            {$currency->sign|escape}</span>
                                        {if $set->total_discount > 0}
                                            <span
                                                class="product_set_buy__price set_old_price">{($set->total_price+$set->total_discount)|convert}
                                                {$currency->sign|escape}</span>
                                        {/if}
                                    ...
                                    <input id='buy_set' type="submit"
                                        class="product_set_buy__button button__product fn_add_set_to_cart product_preview__button"
                                        data-variant_id="set_{$product->variant->id}_{$set->id}"
                                        value="{$lang->simplamarket__sets__set_buy}"
                                        data-result-text="{$lang->simplamarket__sets__set_add}" />
